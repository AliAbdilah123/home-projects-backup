...
func (s *Server) handleBaileysInternalWebhook(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&raw); err != nil {
		_ = s.repo.LogAuditEvent(r.Context(), AuditEntry{Action: "webhook.invalid_payload", TargetType: "webhook_event", Metadata: map[string]any{"provider": whatsappBaileysProvider, "error": "invalid json"}})
	...
	if err := json.Unmarshal(raw, &event); err != nil {
		_ = s.repo.LogAuditEvent(r.Context(), AuditEntry{Action: "webhook.invalid_payload", TargetType: "webhook_event", Metadata: map[string]any{"provider": whatsappBaileysProvider, "error": err.Error(), "payload": string(raw)}})
		http.Error(w, "invalid payload", http.StatusBadRequest)
		return
	}
	if err := validateBaileysEvent(event); err != nil {
		_ = s.repo.LogAuditEvent(r.Context(), AuditEntry{Action: "webhook.invalid_payload", TargetType: "webhook_event", Metadata: map[string]any{"provider": whatsappBaileysProvider, "event_id": event.EventID, "error": err.Error(), "payload": string(raw)}})
